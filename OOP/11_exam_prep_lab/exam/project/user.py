class User:
    ...
    RATE_INCREASE: float = 0.5
    RATE_DECREASE: float = 2.0
    MAX_RATING: float = 10
    ...
    def __init__(self, first_name: str, last_name: str, driving_license_number: str):
        ...
        self.rating = User.INITIAL_RATING
    ...
    def increase_rating(self):
        self.rating += self.RATE_INCREASE

        if self.rating > self.MAX_RATING:
            self.rating = self.MAX_RATING

    def decrease_rating(self):
        expected_rate = self.rating - self.RATE_DECREASE

        if expected_rate < 0:
            self.rating = self.INITIAL_RATING
            self.is_blocked = True
        else:
            self.rating = expected_rate
