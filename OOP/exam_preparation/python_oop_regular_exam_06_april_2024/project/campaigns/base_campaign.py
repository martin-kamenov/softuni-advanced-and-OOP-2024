...
class BaseCampaign(ABC):
    ...
    def __init__(self, campaign_id: int, brand: str, budget: int, required_engagement: float):
    ...
    def campaign_id(self, value: int):
        ...
        if value in BaseCampaign.CAMPAIGN_IDS:
            raise ValueError(f"Campaign with ID {value} already exists. Campaign IDs must be unique.")

        BaseCampaign.CAMPAIGN_IDS.add(value)
        self.__campaign_id = value

    # TODO check if can be formated to be more abstract
    @abstractmethod
